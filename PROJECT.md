...
> **See `README.md` for the full platform architecture** — the stack, per-client infra, compliance design, and enrollment. That file is the source of truth for what Lintra *is*; this file is the source of truth for what the *site* should be. Use the README for accurate technical detail on the Services/About pages.

## What this is

Lintra.cloud is a HIPAA-compliant, open-source managed IT service. This project turns the existing lintra.cloud site into a **content-led front door**: a technical blog that builds trust through demonstrated expertise, with the service offering kept understated in the background.
...
**The platform is general-purpose. The go-to-market is a healthcare spear.** These are not in conflict, and the distinction is load-bearing:

- Lintra the *product* can serve any small business (see README — it's a full MS365/MSP alternative). Keep that capability; don't narrow the platform.
- Lintra the *public positioning and blog* is aimed narrowly and deliberately at **small behavioral health & psychiatric practices**. This is the entry wedge, not the ceiling.

Why narrow on purpose: content only works by ranking and resonating in a space small enough to actually own. "Small business IT" / "open-source MSP" is a saturated bloodbath — unwinnable for an unknown. "HIPAA-compliant Nextcloud for a psychiatry practice" is nearly uncontested, so one good post *is* the top result. The narrowness is the whole reason the acquisition strategy functions. The operator also has real, hard-to-fake credibility in healthcare IT — that's the edge; a generic small-business angle throws it away.

**The plan is narrow-to-broad:** win the healthcare niche first (first 3–5 clients, reputation, search dominance), then let it spill outward via referrals as the general platform picks up adjacent verticals. Do **not** let the blog or homepage drift toward generic "small business" language — that drift breaks the one mechanism that gets Lintra found. Guard the healthcare focus in all public-facing copy.
